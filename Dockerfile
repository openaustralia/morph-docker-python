...
RUN apt-get -y install libxml2-dev libxslt-dev libblas-dev liblapack-dev gfortran libfreetype6-dev libpng-dev

# This needs to be installed first
RUN pip install numpy==1.6.1

ADD pip_requirements2.txt /etc/pip_requirements2.txt
RUN pip install -r /etc/pip_requirements2.txt
